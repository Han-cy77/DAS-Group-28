...
tune_grid <- expand.grid(
  mtry = c(2, 4, 6, 8),               # Number of features to sample at each split
...
for (i in 1:nrow(tune_grid)) {params <- tune_grid[i, ]
  rf_model <- randomForest(
    Income ~ ., 
    data = training_data, 
    ntree = params$ntree, 
...
test_data$predicted_prob <- predict(final_model, newdata = test_data, type = "prob")[, 2]
test_data$predicted_class <- ifelse(test_data$predicted_prob >= 0.5, 1, 0)

# Confusion Matrix
test_data$predicted_class <- factor(
  ifelse(test_data$predicted_prob >= 0.5, ">50K", "<=50K"),
  levels = c("<=50K", ">50K"))
conf_matrix <- confusionMatrix(test_data$predicted_class, test_data$Income)
print(conf_matrix)
...
roc_curve <- roc(test_data$Income, test_data$predicted_prob)
auc_score <- auc(roc_curve)
print(paste("ROC-AUC Score:", auc_score))

# Plot ROC Curve
plot(roc_curve, main = "ROC Curve for Tuned Random Forest", col = "blue", lwd = 2)
